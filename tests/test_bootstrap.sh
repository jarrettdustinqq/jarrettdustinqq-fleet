...
# HTTPS inventory with authenticated gh should configure Git credentials, verify
# every configured repository noninteractively, and avoid creating an unrelated
# SSH key.
...
# If any required repository is inaccessible, bootstrap must fail before the
# first clone/pull mutation. This protects partial controller reconstruction.
